...
module Sinatra
  ...
  def cache_for min 
    set_header 'Cache-Control', "public, max-age=#{min * 60}"
  end
  ...
  def respond_to_request? ctrl_class
    ...
    raise Uni_App::HTTP_404, "Unable to process request: #{response.request_method} #{response.path}"
  end
